...
const Tab: React.FC<TabProps> = ({ index, children, className = '', variant = 'desktop' }) => {
  ...
  const getDesktopClassName = (): string => {
    return cn(
      'data-[state=selected]:text-secondary text-secondary/60 -mb-px cursor-pointer px-10 py-3 focus-visible:outline-0',
      className,
    );
  };
  ...
  const getMobileClassName = (): string => {
    const baseClasses =
      'text-tagline-2 border-stroke-2 min-w-16 cursor-pointer rounded-full border px-3.5 py-2 font-medium transition-all duration-200 ease-out';

    if (isActive) {
      return cn(baseClasses, className);
    }

    return cn(baseClasses, 'text-secondary/60 ', className);
  };
