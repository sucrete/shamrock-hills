...
const BlogCardV5 = ({ blog, className }: BlogCardV5Props) => {
  return (
    ...
    <div>
      <article
        className={cn(
          'rounded-[20px] scale-100 hover:scale-[102%] transition-transform duration-500 hover:transition-transform hover:duration-500 overflow-hidden bg-background-2 flex sm:flex-row flex-col sm:gap-8 gap-0 max-w-[627px] lg:max-w-full lg:mx-0 mx-auto',
          className,
        )}>
        ...
        <div className="sm:py-8 sm:pr-8 p-8 sm:p-0 space-y-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-tagline-2 font-medium text-secondary/60 ">
                {blog.publishDate}
              </span>
            </div>
            <div className="w-px h-[22px] bg-stroke-2 " />
            <div className="flex items-center gap-2">
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" width={24} height={25} viewBox="0 0 24 25" fill="none">
              ...
              <span className="text-tagline-2 font-medium text-secondary/60 ">{blog.readTime}</span>
            </div>
          </div>
          ...
          <div>
            <LinkButton
              href={`/blog/${blog.slug}`}
              className="btn hover:btn-secondary btn-white btn-md w-[85%] md:w-auto mx-auto md:mx-0">
              Read more
            </LinkButton>
          </div>
        </div>
