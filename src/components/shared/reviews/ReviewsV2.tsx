    ...
    <section className={cn(sectionSpacingClass)}>
      ...
      <RevealAnimation delay={0.4}>
        <div className="relative">
          <div className="from-background-3 absolute top-0 left-0 z-40 h-full w-[10%] bg-gradient-to-r to-transparent " />
          <div className="from-background-3 absolute top-0 right-0 z-40 h-full w-[10%] bg-gradient-to-l to-transparent " />
          ...
          <Marquee autoFill pauseOnHover speed={30}>
            <div className="flex items-center gap-8">
              {testimonials.map((testimonial) => (
                <div
                  key={testimonial.id}
                  className="bg-background-1/90 hover:shadow-1 w-full max-w-[358px] min-w-[358px] shrink-0 rounded-[20px] p-8 transition-colors duration-300 ease-linear first:ml-8 hover:bg-white">
                  <p className="line-clamp-3 text-wrap">{testimonial.quote}</p>
                  <div className="bg-stroke-4 my-8 h-px w-full" />
                  <div>
